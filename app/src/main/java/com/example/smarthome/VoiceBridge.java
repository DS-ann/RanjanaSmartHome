...
public final class VoiceBridge implements RecognitionListener {
        ...
        handler.post(new Runnable() {
            @Override public void run() {
                ...
                if (!SpeechRecognizer.isRecognitionAvailable(context)) {
                    postStatus("Speech Recognition Unavailable");
                    return;
                }
                stopping = false;
                running = true;
    ...
    private void startListeningInternal() {
        if (!running || stopping || recognizer == null) return;
    ...
    @Override public void onError(int error) {
        ...
        postStatus("Listening...");
        scheduleRestart(error == SpeechRecognizer.ERROR_RECOGNIZER_BUSY ? 1200L : 250L);
    }
    ...
    private void deliver(android.os.Bundle results, boolean isFinal) {
        ...
        webView.post(new Runnable() {
            @Override public void run() {
                String js = "if(window.onNativeVoiceResult){window.onNativeVoiceResult(" + quote(safeText) + "," + safeFinal + ");}";
    ...
    private void postStatus(final String status) {
        webView.post(new Runnable() {
            @Override public void run() {
                String js = "if(window.onNativeVoiceStatus){window.onNativeVoiceStatus(" + quote(status) + ");}";
                webView.evaluateJavascript(js, null);
            }
        });
    }
